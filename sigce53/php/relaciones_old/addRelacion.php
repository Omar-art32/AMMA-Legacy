		...
		{	
			...
			foreach($arrMarcas as $h => $id)
			{
				...
				if($arr_ext['cont']>0){
					...
					$arr_ins=inserta_relacion($conexion, $cteRec, $cteProv, $tipoR, $marca, $obs, $usr, $arrMarcas);
					if($arr_ins['status']=='OK'){
					   $status="OK";
					   $msj.=$arr_ins['msj'].'<br>';
					}else{
...
function checa_existe($con, $cte_r ,$cte_p, $tip_rel,$mca, $arrM)
{
	try{
		...
		if($count>0) $msj="Ya existe una relacion<b> $relacion </b>entre los asociados:<b> $cte_r </b>y<b> $cte_r </b>".$adMsj;
		return array("status" => 'OK', "cont" => $count,"msj" => $msj);
	}catch (Exception $e){		
		return array("status" => 'error',"cont" => 2, "msj" => $e->getMessage());	
	 }
}
function inserta_relacion($con, $cte_r ,$cte_p, $tip_rel, $mca, $observ, $usuario, $arrM)
{
	try{
		$adMsj="";
