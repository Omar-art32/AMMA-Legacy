<?php
	  ...
	  try {
		  ...
		  $sql_p = "SELECT cr.id,cr.cte_prov, c.nombre,m.marca,m.cve_marca,cr.status,cr.vigencia FROM clientes_relaciones cr INNER JOIN clientes c ON c.no_cliente=cr.cte_prov
		  ...
		  $ps_p->store_result();
          $nr=$ps_p->num_rows;
		  $ps_p->bind_result($id,$prov,$nom_prov,$nom_marca,$let_marca,$status,$vigencia);
		  while($ps_p->fetch())
		  {
			  $estatus=($status!=1)?' <b>(Sin Documento)</b>':'';
			  $vigencia=($vigencia!=1)?' <span class="label label-danger">Vencido</span>':' <span class="label label-success">Vigente</span>';
			  if($tipo_c=='E'){
			  ...
			  $cad_prod.="<tr id='rowRel".$id."'><td>".$prov."</td><td>".utf8_encode($nom_prov).$estatus.$vigencia."</td><td>".$let_marca." - ".utf8_encode($nom_marca)."</td>".$link2.$link."</tr>";
			  //array_push($productores, array("cve_marca" => $row["cve_marca"], "marca" => utf8_encode($row["marca"])));
		  }
		  $ps_p->close();

		  $sql_e = "SELECT cr.id,cr.cte_prov, c.nombre,m.cve_marca cve,m.marca,cr.status,cr.vigencia from clientes_relaciones cr INNER JOIN clientes c ON c.no_cliente=cr.cte_prov
		  ...
		  $ps_e->store_result();
          $nr_e=$ps_e->num_rows;
		  $ps_e->bind_result($id_e,$prov_e,$nom_prov_e,$cve,$n_marca,$status,$vigencia);
		  while($ps_e->fetch())
		  {
			$estatus=($status!=1)?' <b>(Sin Documento)</b>':'';
			$vigencia=($vigencia!=1)?' <span class="label label-danger">Vencido</span>':' <span class="label label-success">Vigente</span>';
			  ...
			  $cad_env.="<tr id='rowRel".$id_e."'><td>".$prov_e."</td><td>".utf8_encode($nom_prov_e).$estatus.$vigencia."</td><td>".$cve.' - '.utf8_encode($n_marca)."</td>".$link2.$link."</tr>";
			  //array_push($productores, array("cve_marca" => $row["cve_marca"], "marca" => utf8_encode($row["marca"])));  
		  }
