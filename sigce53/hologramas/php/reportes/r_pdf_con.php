...
if($res_sum->num_rows>0)
{
	...
	while($row_sum=$res_sum->fetch_assoc())
	{
		...
		$array_resumen[$ind_r]=array($cliente,$marca_hol,$tipo_hol,$row_sum['suma']);
		$ind_r++;
	}
}//FIN OBTENER TOTALES
//print_r($arr_sum);
...
$y=42;
  ...
  if($num_res>0)
  {
	  ...
	  {
	  //---------------INICIO DE VARIABLES-----------
	  $no_cliente=utf8_encode($fila["no_cliente"]);
	  //MARCA
		...
		if($new_pag==0)
		{
		  ...
		  $pdf->SetXY(40,$y);
		   if(strlen($marca)>24)
		   {
			...
			$pdf->MultiCell(40,5,utf8_decode($marca),0,'L',1);
		   }
		   else
		   {
			 $pdf->SetFont('Arial','',8);
		  ...
		  $pdf->SetXY(90,$y);
		  $pdf->Cell(20,$alto_fila,utf8_decode($edo),1,0,'C',1);
		  //categoria
		  $pdf->SetFont('Arial','',6);

		  $pdf->SetXY(110,$y);
		  $pdf->Cell(15,$alto_fila,utf8_decode($categoria),1,0,'C',1);
		  //solicitud
		  $pdf->SetFont('Arial','',7);
		  $pdf->SetXY(125,$y);
