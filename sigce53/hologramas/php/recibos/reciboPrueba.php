...
try
{
    ...
    $y = $y + 2; //18
    $pdf->SetXY(67, $y);
    //$pdf->Cell(90, 5, utf8_decode('Asociación de Maguey y Mezcal Artesanal'), '0', '', 'C'); //CAMBIO
    //CABECERA
    $pdf->SetFont('Calibri-Bold', '', 9);
    $y = $y + 7; //28
    $pdf->SetXY(67, $y);
    //$pdf->Cell(90, 5, utf8_decode('Asociación de Maguey y Mezcal Artesanal A. C.'), 0, '', 'C'); //CAMBIO
    //TITULO AREA
    $pdf->SetFont('Calibri-Bold', '', 16);
    $y = $y + 10; //36
...
function get_marca($cliente, $marca)
{
    ...
    $sql = $conexion->prepare("SELECT marca FROM marcas WHERE substr(no_cliente,1,5)='$cliente' AND cve_marca = '$marca' AND status=1");
