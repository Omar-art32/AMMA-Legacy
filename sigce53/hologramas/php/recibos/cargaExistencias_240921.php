...
try {
	...
	$sql = "SELECT id_existencias,marca,edo,tipo,serie,fol_ini,fol_fin,existencias FROM h_existencias WHERE no_cliente ='{$no_cliente}' and marca='{$marca}'";
	...
	$sql = $conexion->prepare("
		...
		FROM siig.h_salidas hs
		INNER JOIN siig.clientes c ON hs.no_cliente = c.no_cliente
	...
	while ($sql->fetch()) {
				...
				CAST(eh.holograma_inicio AS SIGNED INTEGER) AS INI,  CAST(eh.holograma_fin AS SIGNED INTEGER) AS FIN
					FROM siig.rv_envasado_entrada ee
					LEFT JOIN siig.rv_envasado_holograma eh ON ee.id_envasado_entrada = eh.id_envasado_entrada
	...
	*/
	...
	//echo json_encode(array("status" => "OK", "msj" => "", "estatus"=>$rowm->estatus, "arr_exs" => $arr_exs,"num_res"=>$num_res,  'txth' => $txth ));
}
catch (Exception $e) {
