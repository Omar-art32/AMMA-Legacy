			 ...
			 if(data_r.status==="OK")
			 {
				 if(data_r.num_res>0)
				 {
					  for (var i = 0; i < data_r.arr_exs.length; i++) //RECORRER EL ARRAY arr_exs DEVUELTO Y ASIGNAR LOS ARREGLOS HIJOS A UN OBJETO SEGUN EL NUMERO DE FILAS
					  {
						 ...
						 ctesel = (arr_exist[i].cliente_crm != '' && arr_exist[i].cliente_crm != null)? arr_exist[i].cliente_crm: cte;
						 var cadena = "<tr id='row_Ex_"+i+"' align='center'>";
						 cadena = cadena + "<td>" +ctesel + arr_exist[i].marca + pad(arr_exist[i].fol_ini,7) + arr_exist[i].serie +"</td>";
						 cadena = cadena + "<td>" +ctesel + arr_exist[i].marca + pad(arr_exist[i].fol_fin,7) + arr_exist[i].serie + "</td>";
...
function asignar_folios(cantidad)
{
	...
	arrTemp["tot_mermas"]		=	0;
	arrTemp["motivo_merma"]		=	"";
	arrTemp["mermas_nums"]		=	"";
	arrTemp["mermas_folio"]		=	"";
	arrTemp["nrango"]			=	"";
	if(folios_asign[pos_asign]) {
		
		arrTemp["tot_mermas"]		=	folios_asign[pos_asign].tot_mermas;
		arrTemp["motivo_merma"]		=	folios_asign[pos_asign].motivo_merma;
		arrTemp["mermas_nums"]		=	folios_asign[pos_asign].mermas_nums;
		arrTemp["mermas_folio"]		=	folios_asign[pos_asign].mermas_folio;
		arrTemp["nrango"]			=	folios_asign[pos_asign].nrango;
	...
	folios_asign[pos_asign].cliente_crm		=	arr_exist[pos_asign].cliente_crm;
	if(arrTemp["tot_mermas"] > 0) {
		folios_asign[pos_asign].tot_mermas		=	arrTemp["tot_mermas"];
		folios_asign[pos_asign].motivo_merma	=	arrTemp["motivo_merma"];
		folios_asign[pos_asign].mermas_nums		=	arrTemp["mermas_nums"];
		folios_asign[pos_asign].mermas_folio	=	arrTemp["mermas_folio"];
		folios_asign[pos_asign].nrango			=	arrTemp["nrango"];
	} else {
	...
	ctesel = (arr_exist[pos_asign].cliente_crm != '' && arr_exist[pos_asign].cliente_crm != null)? arr_exist[pos_asign].cliente_crm: cte;
	var cadena = "<tr id='row_Ex_"+pos_asign+"' align='center'>";
    cadena = cadena + "<td>" +ctesel + arr_exist[pos_asign].marca + pad(folios_asign[pos_asign].fol_ini,7) + arr_exist[pos_asign].serie +"</td>";
    cadena = cadena + "<td>" +ctesel + arr_exist[pos_asign].marca + pad(folios_asign[pos_asign].fol_fin,7) + arr_exist[pos_asign].serie + "</td>";
...
function reciboSimple()
{
	...
	$.confirm({
		...
		content: '¿Estás seguro(a) de generar el recibo?',
		type: 'green',
		typeAnimated: true,
		buttons: {
			cancelar: function () {
  ...
  else
  {
	  ...
	  folios_carrito[key_arr].total_sellos=$('#txtTotal').val();
	    ...
	    for(var x=0;x<=pos_asign;x++)
		{
		 ...
		 //console.log(folios_asign[x]);
		 ctesel = (folios_asign[x].cliente_crm != '' && folios_asign[x].cliente_crm != null)? folios_asign[x].cliente_crm: cte;
		 //creamos la cadena con los folios completos
		 var list_fi=ctesel+folios_carrito[key_arr].data[x].marca+pad(folios_carrito[key_arr].data[x].fol_ini,7)+folios_carrito[key_arr].data[x].serie;
	     var list_ff=ctesel+folios_carrito[key_arr].data[x].marca+pad(folios_carrito[key_arr].data[x].fol_fin,7)+folios_carrito[key_arr].data[x].serie;
	  ...
	  for(i=0;i<=pos_asign;i++)//recorrer el arreglo de folios asignados
	  {
		  ...
		  else
		  {
				...
				else
				{
					...
					ctesel = (folios_asign[i].cliente_crm != '' && folios_asign[i].cliente_crm != null)? folios_asign[i].cliente_crm: cte;
					folios_asign[i].mermas_nums=folios_asign[i].mermas_nums+sep+fol_m;
					fol_merma_completo=ctesel+folios_asign[i].marca+pad(fol_m,7)+folios_asign[i].serie;
					...
					$('#fol_ini_mermas').focus();
					//
					folios_asign[i].nrango += (folios_asign[i].nrango != "") ? ", " : "";
					folios_asign[i].nrango += ctesel+folios_asign[i].marca+pad(fol_m,7)+folios_asign[i].serie;
				}
		  }
	  }//loop for
	  // OBTENER SOLO LA CANTIDAD INICIAL DE FOLIOS
	  var nCantidad = parseInt(cantidadInicial) + parseInt(count_mermas)
	  $('#txtCantidad').val(nCantidad);
	  // VALIDAR FOLIOS SUMADOS PARA ENTREGA
	  var cantidad_sol=parseInt($('#txtCantidad').val());
		if(cantidad_sol>gran_total_exist) {
			$('#msjBody').html('Hologramas insuficientes para la cantidad requerida');
			$('#modalMsjs').modal('show');
		...
		else
		{
			...
			for(i=0;i<=pos_asign;i++) {//recorrer el arreglo de folios asignados
				   ...
				   if(folio_i<parseInt(folios_asign[i].fol_ini) || folio_i>parseInt(folios_asign[i].fol_fin)) {
					  if(i===pos_asign)
			          {
						...
						else
						{
							...
							fol_merma_completo=ctesel+folios_asign[i].marca+pad(folio_i,7)+folios_asign[i].serie;
							folios_asign[i].mermas_folio = folios_asign[i].mermas_folio+sep+fol_merma_completo;
							$('#fol_mermas').val($('#fol_mermas').val()+sep+fol_merma_completo);
							subT = folios_asign[i].cant_real;
					        subT=subT-1;
				...
				folios_asign[i].nrango += (folios_asign[i].nrango != "") ? ", " : "";
				folios_asign[i].nrango += ctesel+folios_asign[i].marca+pad(fol_m1,7)+folios_asign[i].serie + " - " + ctesel+folios_asign[i].marca+pad(fol_m2,7)+folios_asign[i].serie;
			}//loop for folio
			$('#fol_ini_mermas').val('');
			$('#fol_fin_mermas').val('');
			$('#fol_ini_mermas').focus();
