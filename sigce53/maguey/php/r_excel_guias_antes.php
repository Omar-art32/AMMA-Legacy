...
{*/
	...
	{*/
		...
		//if(isset($_POST['fechaini'])){
			...
			$cadenaSql = "SELECT c.*, hev.*, c.id_extraccion cid_extraccion, p.paraje, pe.tapada, pe.lts_producidos, p.maguey_con_registro, p.servicio, p.id_cliente pid_cliente 
			FROM paraje p 
			...
			//echo $cadenaSql;
				...
				->setCellValue('D'.$r_h, 'Fecha')
				->setCellValue('E'.$r_h, 'Estado')
				->setCellValue('F'.$r_h, 'Tipo de Guía')
				->setCellValue('G'.$r_h, '# Cliente')
				->setCellValue('H'.$r_h, 'Extracción')
				->setCellValue('I'.$r_h, 'Tapada')
				->setCellValue('J'.$r_h, 'Litros Producidos');
				...
				foreach($res as $row) {
					...
					foreach ($infoCampo as $valor) {
						...
						if($nameC != "poligono") {
							...
							$objPHPExcel->setActiveSheetIndex(0)->setCellValue($letrita.$filan, $row["$nameC"]);
							$letrita++;
						}
					}
					...
					$letrita++;
					$paraje = $row["paraje"];
					$objPHPExcel->setActiveSheetIndex(0)->setCellValue($letrita.$filan, $paraje);
					$letrita++;
					$objPHPExcel->setActiveSheetIndex(0)->setCellValue('A'.$filan, $row["cid_extraccion"]);
				...
				$dir_file="http://".$svr_dir."/sigce/maguey/tmp_excel/".$file_name;
